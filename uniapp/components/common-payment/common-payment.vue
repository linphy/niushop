								...
								<view class="goods-info">
									<view class="top-wrap">
										...
										<block v-if="goodsItem.is_virtual == 0">
											<view class="error-tips" v-if="orderCreateData.delivery && goodsItem.support_trade_type && goodsItem.support_trade_type.indexOf(orderCreateData.delivery.delivery_type) == -1">
												<text class="iconfont icon-gantanhao"></text> 
				...
				<view class="site-wrap box member-card-wrap" v-if="paymentData.recommend_member_card">
					...
					<view class="body" v-if="orderCreateData.is_open_card">
						<view class="item" :class="{'active color-base-border': item.key == orderCreateData.member_card_unit }" v-for="(item, index) in cardChargeType" :key="index" @click="selectMembercardUnit(item.key)">
							...
							<text class="iconfont icon-icon color-base-text price-font"></text>
						</view>
					</view>
				</view>
						...
						<view class="popup-body store-popup" :class="{ 'safe-area': isIphoneX }">
							<view class="delivery-content">
								<view class="item-wrap" v-for="(item, index) in storeList" :key="index" @click="selectPickupPoint(item)">
									...
									<view class="icon" v-if="item.store_id == orderCreateData.delivery.store_id"><text class="iconfont icon-yuan_checked color-base-text"></text></view>
								</view>
