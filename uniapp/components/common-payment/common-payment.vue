<template>
	<view class="order-container" :class="{ 'safe-area': isIphoneX }">
		<template v-if="paymentData">
			...
			<template v-else>
				<view class="address-box" v-if="orderCreateData.delivery.delivery_type == 'express'">
					<view class="info-wrap" v-if="memberAddress" @click="selectAddress">
						...
						<view class="content">
							<text class="name font-size-base">{{ memberAddress.name ? memberAddress.name : '' }}</text>
							<text class="font-size-base">{{ memberAddress.mobile ? memberAddress.mobile : '' }}</text>
					...
					<view class="empty-wrap" v-else @click="selectAddress">
						<view class="icon-wrap empty"><view class="icon"><image :src="$util.img('public/uniapp/order/icon-address.png')" mode="widthFix"></image></view></view>
				...
				<view class="address-box" v-if="orderCreateData.delivery.delivery_type == 'local'">
					<view class="info-wrap" v-if="localMemberAddress" @click="selectAddress">
						...
						<view class="content">
							<text class="name font-size-base">{{ localMemberAddress.name ? localMemberAddress.name : '' }}</text>
							<text class="font-size-base">{{ localMemberAddress.mobile ? localMemberAddress.mobile : '' }}</text>
							<view class="desc-wrap">
			...
			<view class="site-wrap order-goods">
				<view class="site-body">
					<!-- 商品 -->
					<view class="goods-wrap" v-for="(goodsItem, goodsIndex) in goodsData.goods_list" :key="goodsIndex">
					...
					<view class="order-cell textarea-box">
						...
						<!-- <view class="box"><textarea type="text" auto-height="true" maxlength="100" placeholder="留言前建议先与商家协调一致" placeholder-class="color-tip" v-model="orderCreateData.buyer_message" /> </textarea></view> -->
			...
			<view class="site-wrap" v-if="
					...
					goodsData.invoice ||
					paymentData.recommend_member_card
				">
				<view class="site-footer">
					...
					<view class="order-cell clear-flex" v-if="paymentData.recommend_member_card">
						...
						<view class="box member-card-wrap">
							<view class="head" @click="selectMemberCard">
								...
								<view class="info">开通{{ paymentData.recommend_member_card.level_name }}<text>本单预计可省{{ paymentData.recommend_member_card.discount_money|moneyFormat }}元</text></view>
								<text class="iconfont" :class="orderCreateData.is_open_card == 1 ? 'iconyuan_checked color-base-text' : 'iconyuan_checkbox'"></text>
							</view>
			...
			<template v-if="calculateData">
				...
				<view v-if="transactionAgreement.title && transactionAgreement.content" class="agreement">
					购买前请先阅读<text @click="$refs.agreementPopup.open()">《{{transactionAgreement.title}}》</text>，下单即代表同意该协议
				</view>
						...
						<view>
							...
							<block v-if="orderCreateData.is_invoice == 1">
								<view class="invoice-cell">
									...
									<view class="option-grpup">
										<view class="option-item" :class="{ 'color-base-bg active': orderCreateData.invoice_type == item }" @click="changeInvoiceType(item)" v-for="(item, index) in goodsData.invoice.invoice_type.split(',')" :key="index">{{item == 1?'纸质':'电子'}}</view>
									</view>
								</view>
								<view class="invoice-cell">
				...
				<view class="promotion-popup popup">
					...
					<scroll-view scroll-y="true" class="popup-body" :class="{ 'safe-area': isIphoneX }">
						<view class="order-cell" style="align-items: baseline;">
							<view class="tit"><text class="promotion-mark ns-gradient-promotionpages-payment">{{ promotionInfo.title }}</text></view>
			...
			<uni-popup ref="couponPopup" type="bottom" v-if="calculateGoodsData && calculateGoodsData.coupon_list &&calculateGoodsData.coupon_list.length" :mask-click="false">
				<view class="coupon-popup popup" @touchmove.prevent.stop>
					...
					<scroll-view scroll-y="true" class="popup-body" :class="{ 'safe-area': isIphoneX }">
						<view
							class="coupon-item"
							v-for="(couponItem, couponIndex) in calculateGoodsData.coupon_list"
							:key="couponIndex"
			...
			<view @touchmove.prevent>
				<uni-popup ref="agreementPopup" type="center" :maskClick="false">
					<view class="agreement-conten-box">
						<view class="close"><text class="iconfont iconclose" @click="$refs.agreementPopup.close()"></text></view>
