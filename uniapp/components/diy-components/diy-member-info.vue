				...
				<view class="info-wrap" v-if="memberInfo">
					...
					<view class="info">
						...
						<view
							...
							@click="redirectBeforeAuth(memberInfo.member_level_type ? '/pages_tool/member/card' : '/pages_tool/member/level')"
						>
							<image :src="$util.img('public/uniapp/member/supervip_icon.png')" mode="widthFix" class="level-icon"></image>
							<view class="level-name">{{ memberInfo.member_level_name }}</view>
						</view>
						<view
							class="member-level"
							v-if="value.style == 4 && memberInfo.member_level"
							@click="redirectBeforeAuth(memberInfo.member_level_type ? '/pages_tool/member/card' : '/pages_tool/member/level')"
						>
				...
				<view class="member-info-style4" v-show="value.style == 4">
					...
					<view class="style4-other">
						<view class="style4-btn-wrap">
							<button type="primary" @click="redirect('/pages_tool/recharge/list')">余额充值</button>
							<ns-contact><button type="primary" plain>专属顾问</button></ns-contact>
						</view>
						<image
							class="code"
							@click="redirect('/pages_tool/store/payment_qrcode')"
							:src="$util.img('app/component/view/member_info/img/style_4_code.png')"
							mode="aspectFill"
						></image>
					</view>
				</view>
	...
	computed: {
		memberInfostyle() {
			...
			if (this.value.theme == 'default') {
				...
				style.background = `url('${img}') no-repeat bottom / ${backSize},linear-gradient(${this.value.gradientAngle}deg, ${this.value.bgColorStart} 0%, ${
