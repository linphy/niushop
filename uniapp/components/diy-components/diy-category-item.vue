<template>
	<view class="item-wrap" :class="type">
		<block v-if="type == 'category' && category.child_list.length">
			<view class="category-adv" v-if="category.image_adv" @click="$util.redirectTo('/pages/goods/list', {category_id: category.category_id})">
	...
	export default {
		...
		watch: {
			'cartList': function(nval){
				let cart = {}, cartList = Object.keys(this.$store.state.cartList);
				if (cartList.length) {
		...
		methods:{
			...
			increase(event, data){
				...
				let cart = this.cart['goods_' + data.goods_id] && this.cart['goods_' + data.goods_id]['sku_' + data.sku_id] ? this.cart['goods_' + data.goods_id]['sku_' + data.sku_id] : null,
					num = cart ? cart.num : 0,
