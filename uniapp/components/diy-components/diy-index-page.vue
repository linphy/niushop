						...
						<view class="category-goods" v-show="!isloading">
							...
							>
								<block slot="list">
									...
									<!-- 分类广告 -->
									<image class="category_adv" v-if="cateList[pageIndex].image_adv" :src="$util.img(cateList[pageIndex].image_adv)" mode="widthFix"></image>
									...
									<view v-if="!isloading && goodsList[pageIndex].list.length == 0"><ns-empty text="该分类下暂无商品" :isIndex="false"></ns-empty></view>
								</block>
							</mescroll-uni>
		...
		setModuleLocatinoFn() {
			...
			query
				...
				.boundingClientRect(data => {
					...
					this.$store.commit('setDiyGroupPositionObj', { diyIndexPage: diyIndexPage });
				})
				.exec();
		},
		showModuleFn() {
...
.nav-top-category {
	...
	.unfold-arrows {
		...
		&::after {
			...
			left: 12rpx;
			width: 2rpx;
			box-shadow: -16rpx 0 8rpx rgba(0, 0, 0, 0.8);
			transform: scale(0.6);
		}
	}
}
