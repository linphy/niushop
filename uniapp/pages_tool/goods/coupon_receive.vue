	...
	onShow() {
		...
		if (uni.getStorageSync('token')) {
			this.$util.getMemberId().then(resolve => {
				...
				if (this.info.coupon_type_id) this.setPublicShare();
			});
			this.getReceivedNum();
		}
		//记录分享关系
	...
	methods: {
		...
		setPublicShare(){
			...
			if (this.info.range_type == 1) {
				var desc = `满${this.info.at_least}全场可用`;
			} else {
				var desc = `满${this.info.at_least}部分商品可用`;
			}
			this.$util.setPublicShare({
				title: title, 
				desc: desc,
				link: shareUrl,
				imgUrl: this.$store.state.siteInfo ? this.$store.state.siteInfo.logo_square : ''
			})
		}
	},
	computed: {
