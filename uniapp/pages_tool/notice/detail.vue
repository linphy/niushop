...
export default {
	...
	onLoad(options) {
		options.notice_id ? (this.noticeId = options.notice_id) : this.$util.redirectTo('/pages_tool/notice/list', {}, 'redirectTo');
	},
	onShow() {
		this.getData();
	},
	...
	onShareTimeline() {
		...
		var query = '/pages_tool/notice/notice_id?id=' + this.noticeId;
		return {
			title: title,
			query: query,
			imageUrl: ''
