	...
	<view class="detail-container" :class="{ 'safe-area': isIphoneX }">
		...
		<view class="status-wrap color-base-bg" :style="{ backgroundImage: 'url(' + $util.img('public/uniapp/order/status-wrap-bg.png') + ')' }">
			...
			<view class="order-time" v-if="orderData.order_status == 0" id="action-date">
				剩余时间：
				<uni-count-down
					:day="orderData.closeTimeMachine.d"
					:hour="orderData.closeTimeMachine.h"
					:minute="orderData.closeTimeMachine.i"
					:second="orderData.closeTimeMachine.s"
					color="#fff"
					splitorColor="#fff"
				/>
			</view>
		...
		<template v-if="orderData.order_type == 2">
			...
			<view class="store-wrap">
				...
				<view class="pick-block first-pick-block" v-if="">
					...
					<block v-if="orderData.buyer_ask_delivery_time && orderData.buyer_ask_delivery_time != 0">
						<input
							class="last-child"
							disabled
							:value="
								orderData.buyer_ask_delivery_time.indexOf('-') != -1
									? orderData.buyer_ask_delivery_time
									: $util.timeStampTurnTime(orderData.buyer_ask_delivery_time)
							"
						/>
					</block>
					<block v-else><input class="last-child" disabled :value="'立即自提'" /></block>
				</view>
			</view>
			...
			<view class="pickup-info" v-if="orderData.pay_status">
				<view class="pickup-code-info">
					...
					<view class="code">
						<image :src="$util.img(orderData.pickup_barcode)" @click="previewMedia($util.img(orderData.pickup_barcode))" class="barcode" mode="widthFix"></image>
						<image :src="$util.img(orderData.pickup)" @click="previewMedia($util.img(orderData.pickup))" class="qrcode" mode="widthFix"></image>
					</view>
				</view>
			</view>
		</template>
				...
				<view v-for="(goodsItem, goodsIndex) in orderData.order_goods" :key="goodsIndex" class="goods-item">
						...
						<view class="goods-info" @click="goDetail(goodsItem)">
							...
							<view v-if="goodsItem.card_item_id" class="goods-card">
								...
								<text class="goods-price ">
									...
									<text class="price-style large">
										{{
											parseFloat(goodsItem.card_promotion_money)
												.toFixed(2)
												.split('.')[0]
										}}
									</text>
					...
					<view class="goods-action">
						<block v-if="orderData.is_enable_refund">
							<view
								@click="goRefund(goodsItem.order_goods_id)"
			...
			this.$api.sendRequest({
				...
				success: res => {
					if (res.code >= 0) {
						...
						if (res.data.buyer_ask_delivery_time && res.data.buyer_ask_delivery_time.indexOf('立即配送') != -1)
							res.data.buyer_ask_delivery_time = res.data.buyer_ask_delivery_time.replace('立即配送', '');
						let num = 0;
		...
		operation(action) {
			switch (action) {
				...
				case 'memberTakeDelivery': //收货
					...
					break;
				case 'trace': //查看物流
					this.$util.redirectTo('/pages_tool/order/logistics', { order_id: this.orderData.order_id });
					break;
				case 'memberOrderEvaluation': //评价
					this.$util.redirectTo('/pages_tool/order/evaluate', { order_id: this.orderData.order_id });
					break;
				//批量退款
				case 'memberBatchRefund':
					this.$util.redirectTo('/pages_tool/order/refund_type_select', { order_id: this.orderId });
					break;
