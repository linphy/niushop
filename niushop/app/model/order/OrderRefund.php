    ...
    public function finishAction($order_goods_id, $log_data = [],$is_deposit_back = 1, $is_active_refund = false)
    {
        ...
        try {
                ...
                if($order_goods_info['refund_money_type'] == 1){
                    ...
                    if ($refund_balance_money > 0) {
                        ...
                        if ($order_use_balance_money > $refunded_balance_money) {
                            ...
                            $balance_result = $member_account_model->addMemberAccount($order_info[ 'site_id' ], $order_info[ 'member_id' ], 'balance_money', $refundable_balance_money, 'refund', $order_goods_info[ 'order_id' ], '订单退款返还');
                        ...
                        if ($refund_balance_money > 0) {
                            $balance_result = $member_account_model->addMemberAccount($order_info[ 'site_id' ], $order_info[ 'member_id' ], 'balance', $refund_balance_money, 'refund', $order_goods_info[ 'order_id' ], '订单退款返还');
                            if ($balance_result[ 'code' ] < 0) {
                                model('order_goods')->rollback();
                                return $balance_result;
                            }
                    ...
                    $member_account_model = new MemberAccount();
                    $refund_result = $member_account_model->addMemberAccount($order_info[ 'site_id' ], $order_info[ 'member_id' ], 'balance', $refund_total_real_money, 'refund', $order_goods_info[ 'order_id' ], '订单退款返还');
            ...
            if ($order_goods_info['refund_mode'] == 1 && $order_goods_info[ 'use_point' ] > 0) {
                $member_account_model = new MemberAccount();
                $point_result = $member_account_model->addMemberAccount($order_info[ 'site_id' ], $order_info[ 'member_id' ], 'point', $order_goods_info[ 'use_point' ], 'refund', $order_goods_info[ 'order_id' ], '订单退款返还');
            ...
            if ($order_goods_count == $refund_count && $order_info['order_status'] != Order::ORDER_COMPLETE) {
                ...
                $close_result = $order_common_model->orderClose($order_goods_info[ 'order_id' ], [], '退款完成,订单关闭');
