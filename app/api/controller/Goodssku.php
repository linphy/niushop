...
class Goodssku extends BaseApi
{
    ...
    public function pageComponents()
    {
        ...
        if (!empty($list[ 'data' ][ 'list' ]) && $token[ 'code' ] >= 0) {
            $list[ 'data' ][ 'list' ] = $goods->getGoodsListMemberPirce($list[ 'data' ][ 'list' ], $this->member_id);
        }
    ...
    public function recommend()
    {
        ...
        $condition[] = [ 'gs.site_id', '=', $this->site_id ];
        $goods = new Goods();
        $field = 'g.sale_show, gs.goods_id,gs.sku_id,gs.sku_name,gs.price,gs.market_price,gs.discount_price,gs.stock,(g.sale_num + g.virtual_sale) as sale_num,gs.sku_image,gs.goods_name,gs.site_id,gs.is_free_shipping,gs.introduction,gs.promotion_type,g.goods_image,gs.unit,g.label_name';
        $alias = 'gs';
        ...
        $is_show = $guess_you_like[ $route ] ?? 0;
        if ($guess_you_like[ 'type_show' ] == 2 && $is_show == 1) {
            $join = [
                [ 'goods g', 'gs.sku_id = g.sku_id', 'inner' ],
