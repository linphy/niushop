...
$(function () {
	...
	layui.use(['element', 'laytpl', 'form', 'upload'], function () {
		...
		form.verify({
			...
			min_buy: function(value){
				if (value.length > 0) {
					...
					if (value > $('[name="max_buy"]').val() && $('[name="max_buy"]').val() > 0) {
						element.tabChange('goods_tab', "basic");
						return '起售数量不能大于限购数量';
					}
				}
		...
		form.on('submit(save)', function (data) {
			...
			$(".ns-attr-new .goods-new-attr-tr").each(function() {
				var attr_name = $(this).find(".add-attr-name").val();
				var attr_value = $(this).find(".add-attr-value").val();
				var attr = {};
				if (attr_name != "" && attr_value != "") {
					attr.attr_class_id = -Math.ceil(Math.random()*10);
					attr.attr_id = -Math.ceil(Math.random()*10);
					attr.attr_name = attr_name;
					attr.attr_value_id = -Math.ceil(Math.random()*10);
		...
		var layerIndex = layer.open({
			...
			yes: function () {
				...
				$(".layui-block").each(function() {
					...
					if (cate_id_1 == id_1 && cate_id_2 == id_2 && cate_id_3 == id_3 && i != j) {
						bool = true;
					}
				});
...
function isNullTable() {
	...
	if (len == 0) {
		$(".ns-attr-new").html('<tr class="ns-null-data"><td colspan="3" align="center">无数据</td></tr>');
	} else {
