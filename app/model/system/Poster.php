...
//                $poster_height = !empty($member_info) ? 1120 : 1000;
                ...
                if(!empty($poster_data['data']['background'])){
                    list($width,$height,$type,$attr)=getimagesize(getUrl().$poster_data['data']['background']);

                    $height = 720*$height/$width;
                    $back_ground = [
                        'action' => 'imageCopy', // 写入背景图
                        'data'   => [
                            getUrl().$poster_data['data']['background'],
                            0,
                            0,
                            $poster_width,
                            $poster_height,
                ...
                $option = [
                    ...
                    [
                        ...
                        'data'   => [
                            !empty($site_info['data']['logo']) ? $site_info['data']['logo']: getUrl() . '/app/shop/view/public/img/shop_logo.png',
            ...
            $res = $option_res->jpeg('upload/poster/goods', 'goods_' . $promotion_type . '_' . $qrcode_param['sku_id'] . '_' . $qrcode_param['source_member'] . '_' .time().'_' . $app_type);
            if ($res['code'] == 0) {
    ...
    {
        $info = model('goods_sku')->getInfo(['sku_id' => $sku_id], 'sku_name,introduction,price,discount_price,sku_image,collect_num,template_id,market_price');
        return $info;
    }
