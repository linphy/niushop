...
class Verify extends BaseModel
{
     ...
     * @param unknown $data
     */
    public function addVerify($type, $site_id, $site_name, $content_array, $expire_time = 0)
    {
        $code       = $this->getCode();
    ...
    public function verify($verifier_info, $code)
    {
        ...
        try {
            $verify_info = model("verify")->getInfo([['verify_code', '=', $code]], 'id, verify_code, verify_type, verify_type_name, verify_content_json, verifier_id, verifier_name, is_verify, expire_time');
