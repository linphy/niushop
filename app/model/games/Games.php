...
class Games extends BaseModel
{
    ...
    public function addGames($game_data, $award_json)
    {
        ...
        try {
            ...
            if ($game_data['status'] == 1) {//进行中
                ...
                $cron->addCron(1, 0, "游戏活动关闭", "CloseGames", $game_data['end_time'], $game_id);
            } else {//未进行
                $cron->addCron(1, 0, "团购活动开启", "OpenGames", $game_data['start_time'], $game_id);
                $cron->addCron(1, 0, "团购活动关闭", "CloseGames", $game_data['start_time'], $game_id);
            }
     ...
     * @return array
     */
    public function editGames($condition, $game_data, $award_json)
    {
        ...
        try {
            ...
            $cron->deleteCron([['event', '=', 'OpenGames'], ['relate_id', '=', $game_id]]);
            $cron->deleteCron([['event', '=', 'CloseGames'], ['relate_id', '=', $game_id]]);
