var manyGoodsListHtml = '<div style="display:none;"></div>';

Vue.component("many-goods-list-sources", {
	...
	methods: {
		verify : function (index) {
			...
			for (var i=0; i<vue.data[index].list.length; i++) {
				...
				if (vue.data[index].list[i].sources == 'category' && vue.data[index].list[i].categoryId == 0){
					res.code = false;
					res.message = "请选择商品分类";
				}
				if (vue.data[index].list[i].sources == 'diy' && vue.data[index].list[i].goodsId.length == 0){
					res.code = false;
					res.message = "请选择商品";
				}
			}
		...
		addGoods: function(index) {
			...
			}, self.$parent.data.list[index].goodsId, {mode: "spu", disabled: 0, promotion: "module", post: ns.appModule});
		},
		selectCategory(i){
			var self = this;
			layui.use(['form'], function(){
        		...
        		layer.open({
					...
					success: function(layero, index) {
		                ...
		                $("input[name='category_select_id']:checked").prop("checked",false);
		                if (self.data.list[i].categoryId) {
	                		$('.layui-layer-content [data-category_select_id="'+ self.data.list[i].categoryId +'"]').prop("checked", true);
		                }
		                form.render();
					},
					yes: function (index, layero) {
						...
						self.data.list[i].categoryName =  selected.parents('tr').find('.category-name').text();
						self.data.list[i].categoryId = selected.attr('data-category_select_id');
						layer.closeAll()
					}
				});
			})
