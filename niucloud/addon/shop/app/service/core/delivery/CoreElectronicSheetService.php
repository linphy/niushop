            ...
            ->with(
                [
                    'order_goods' => function($query) {
                        $query->field('order_id, sku_id, goods_name, sku_name, num,status,delivery_id');
                    }
                ])->select()->toArray();
        ...
        if ($params[ 'print_type' ] == 'single') {
            ...
            foreach ($params[ 'list' ] as $k => $v) {
                ...
                foreach ($order_list as $ck => $cv) {
                    ...
                    $key = $es_template_info[ 'company' ][ 'express_no_electronic_sheet' ] . '_' . $cv[ 'order_no' ] . '_' . $es_config[ 'kdniao_id' ] . '_' . $es_template_info[ 'print_style' ] . '_' . $v[ 'delivery_id' ] . '_' . $k;
                    $cache = Cache::get($key);
                    if (!empty($cache)) {
            ...
            foreach ($order_list as $k => $v) {
                ...
                $key = $es_template_info[ 'company' ][ 'express_no_electronic_sheet' ] . '_' . $v[ 'order_no' ] . '_' . $es_config[ 'kdniao_id' ] . '_' . $es_template_info[ 'print_style' ] . '_' . $k;
                $cache = Cache::get($key);
                if (!empty($cache)) {
                    $result[] = $cache;
                    continue;
