<template>
    <view class="discount bg-[#f6f6f6] min-h-[100vh]" :style="themeColor()">
        ...
        <view class="fixed top-0 left-0 w-full z-10 text-[0]">
			<!-- #ifdef MP-WEIXIN -->
			<view class="absolute top-0 left-0 z-999">
				<top-tabbar :data="param" titleColor="#fff" class="top-header"/>
			</view>
			
			<!-- #endif -->
			<u-swiper v-if="bannerList.length" :list="imgList" :indicator="bannerList.length" :indicatorStyle="{'bottom': '50rpx',}" :autoplay="true" height="490rpx" @click="toRedirect"></u-swiper>
						...
						<view class="ml-[20rpx] flex-1 flex flex-col justify-between">
							...
							<view class="w-full h-[78rpx] bg-[#FFECEC] flex justify-between rounded-[40rpx]">
								<view class="mr-[20rpx] pl-[29rpx] flex-1 flex flex-col justify-center">
									<view class="text-[#EF000C] flex items-center mb-[10rpx]">
										<view class="h-[40rpx]">
									...
									<view class="flex items-center mt-[-5rpx]">
										<view class="w-[25rpx] h-[25rpx] mr-[2rpx] rounded-[12.5rpx] text-center text-[#fff] bg-[#EF000C] flex items-center justify-center" :class="{'!bg-[#FF8540]':item.activeGoods.active_goods_status!='active'}">
											<text class="text-[10rpx] iconzhekou iconfont"></text>
										</view>
										<view class="text-[18rpx] font-400 text-[#EF000C] leading-[25rpx]" :class="{'!text-[#FF8540]':item.activeGoods.active_goods_status!='active'}">已省{{item.goodsSku.active_reduce_money}}元</view>
									</view>
								</view>
