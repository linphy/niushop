<template>
	<view class="bg-[#F6F6F6] min-h-[100vh] overflow-hidden" :style="themeColor()">
			...
			<view class="py-[20rpx] sidebar-marign">
				<template v-for="(item, index) in list">
					<view v-if="item.btnType === 'collected'"
							class="flex items-center relative w-[100%] rounded-[20rpx] overflow-hidden bg-[#fff] py-[20rpx] background-size"
						...
						<view class="box-border flex-1 border-0 border-r-[1px] border-[#FFDCDC] border-dashed flex items-center">
							<view class="w-[164rpx] box-border flex justify-center ">
								<view class="flex items-baseline text-[var(--price-text-color)]">
									...
									<text class="text-[54rpx] font-500 text-left leading-[70rpx] max-w-[136rpx] truncate price-font">{{ item.coupon_price }}</text>
								</view>
							</view>
							<view class="flex-1 box-border ml-[10rpx]">
								...
								<view class="mt-[10rpx] text-left flex items-center">
									<text class="bg-[#FFEFF0] whitespace-nowrap text-[#EF000C] text-[20rpx] h-[28rpx] leading-[28rpx] px-[10rpx] rounded-[13rpx] mr-[10rpx] flex-shrink-0">{{ item.type_name }}</text>
									<text class="text-[22rpx] truncate max-w-[190rpx] leading-[30rpx] text-[#626779]">{{ item.title }}</text>
								</view>
								<view class="w-[100%] mt-[6rpx] text-[22rpx] leading-[30rpx] text-[#626779]">
					...
					<view v-else class="flex items-center relative w-[100%] rounded-[20rpx] overflow-hidden bg-[#fff] py-[20rpx] background-size" :class="{'mt-[20rpx]':index}" @click="toDetail(item.id)">
						...
						<view v-if="item.btnType === 'collecting'" @click.stop="collecting(item.id, index)" class="pr-[20rpx] pl-[34rpx]">
							<button :style="{width:'144rpx',height:'56rpx',color:'#fff', fontSize:'24rpx',lineHeight:'56rpx', padding:'0', backgroundColor:'var(--primary-color)',border:'none',borderRadius:'30rpx'}" >立即领取</button>
						</view>
						<view v-if="item.btnType === 'using'" @click.stop="toLink(item.id)" class="pr-[20rpx] pl-[34rpx]">
							<button :style="{width:'144rpx',height:'56rpx',color:'var(--primary-color)', fontSize:'24rpx',lineHeight:'52rpx', padding:'0',backgroundColor:'transparent',border:'2rpx solid var(--primary-color)',borderRadius:'30rpx'}"  >去使用</button>
						</view>
						<view class="absolute top-0 right-[190rpx]  h-[10rpx] w-[20rpx] rounded-br-[20rpx] rounded-bl-[20rpx] bg-[#F6F6F6] "></view>
						<view class="absolute bottom-0 right-[190rpx] h-[10rpx] w-[20rpx] rounded-tr-[20rpx] rounded-tl-[20rpx] bg-[#F6F6F6]"></view>
					</view>
...
const mescrollTop = computed(() => {
	let style = ''
	style = Object.keys(menuButtonInfo).length ? (pxToRpx(Number(menuButtonInfo.height)) + pxToRpx(menuButtonInfo.top) + pxToRpx(8) +  416) + 'rpx' : '416rpx'
	return style
})
