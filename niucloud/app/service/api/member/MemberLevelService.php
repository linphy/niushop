...
class MemberLevelService extends BaseApiService
{
    ...
    public function getList(){
        $list = (new MemberLevel())->where([ ['level_id', '>', 0] ])->field('level_id,level_name,growth,remark,level_benefits,level_gifts')->order('growth asc')->select()->toArray();
        ...
        if (!empty($list)) {
            foreach ($list as $k => $item) {
                ...
                }, CoreMemberService::getBenefitsContent($item['level_benefits'], 'member_level')));
                if (!empty($item['level_gifts'])) $list[$k]['level_gifts'] = CoreMemberService::getGiftContent($item['level_gifts'], 'member_level');
                ...
                if ($length < 4) {
                    foreach ($filling as $key => $content) {
                        if (is_array($list[$k]['level_benefits']) && count($list[$k]['level_benefits']) == 4) break;
