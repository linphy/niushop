...
class CorePayService extends BaseCoreService
{
     ...
     * @return string|null
     */
    public function create(string $main_type, int $main_id, float $money, string $trade_type, int $trade_id, string $body)
    {
     ...
     */
    public function findPayInfoByTrade(string $trade_type, int $trade_id)
    {
        $where = array(
            [ 'trade_type', '=', $trade_type ],
            [ 'trade_id', '=', $trade_id ],
     ...
     * @return Pay|array|mixed|Model
     */
    public function findPayInfoByOutTradeNo(string $out_trade_no)
    {
        $where = array(
            [ 'out_trade_no', '=', $out_trade_no ]
        );
     ...
     */
    public function getInfoByOutTradeNo(string $out_trade_no, string $channel)
    {
        $where = array(
            [ 'out_trade_no', '=', $out_trade_no ]
        );
     ...
     * @throws ModelNotFoundException
     */
    public function getPayTypeByTrade(string $trade_type, string $channel)
    {
        //todo  校验场景控制支付方式
        return array_values(( new CorePayChannelService() )->getAllowPayTypeByChannel($channel, $trade_type));
    }
     ...
     */
    public function checkOrCreate($trade_type, $trade_id)
    {
        $pay = $this->findPayInfoByTrade($trade_type, $trade_id);
        if ($pay->isEmpty()) {
            //创建新的支付单据
...
//        if ($pay['status'] == PayDict::STATUS_CANCLE) throw new PayException('PAY_IS_REMOVE');
     ...
     * @return bool
     */
    public function returnTo($pay_item)
    {
...
//        if ($pay['status'] == PayDict::STATUS_FINISH) throw new PayException('DOCUMENT_IS_PAID');//当前单据已支付
     ...
     * @return true
     */
    public function payNotify(string $out_trade_no, string $type, array $params = [])
    {
        $pay = $this->findPayInfoByOutTradeNo($out_trade_no);
...
//            }
    ...
    public function payClose($data)
    {
        ...
        try {
            ...
            $result = event('PayClose', [ 'out_trade_no' => $out_trade_no, 'trade_type' => $pay->trade_type, 'trade_id' => $pay->trade_id ]);
            if (!check_event_result($result)) {
                Db::rollback();
                return false;
            }
