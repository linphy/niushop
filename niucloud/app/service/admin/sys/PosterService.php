...
class PosterService extends BaseAdminService
{
    ...
    {
        $field = 'id, name, type, channel, value, status,is_default, create_time, update_time, addon';
        $order = "update_time desc";

        $search_model = $this->model->where([ [ 'id', '>', 0 ] ])->withSearch([ 'name', 'type' ], $where)->field($field)->order($order)->append([ 'type_name' ]);
    ...
    {
        $order = "update_time desc";
        return $this->model->where([ [ 'id', ">", 0 ] ])->withSearch([ "name", 'type' ], $where)->field($field)->order($order)->select()->toArray();
    }
