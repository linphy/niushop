...
class PrinterTemplateService extends BaseAdminService
{
    ...
    public function getPage(array $where = [])
    {
        ...
        $search_model = $this->model->where([ [ 'template_id', ">", 0 ] ])->withSearch([ "template_id", "template_type", "template_name" ], $where)->field($field)->order($order)->append([ 'template_type_name' ]);
    ...
    {
        $field = 'template_id,template_type,template_name,value';
    ...
    public function del(int $id)
    {
        ...
        $model = $this->model->where([ [ 'template_id', '=', $id ] ])->find();
        $res = $model->delete();
        return $res;
    }
